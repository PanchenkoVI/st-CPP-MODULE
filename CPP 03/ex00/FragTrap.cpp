...
FragTrap::FragTrap(FragTrap const & aft): _name(aft._name) {

	std::cout << RED << "|------------------------------|__OTHER__|----------------------------->\n" <<  END;
	_name = aft._name;
	_level = aft._level;
...
FragTrap::~FragTrap(){
	...
	std::cout << GREEN << "		      Goodbye my friend " << this->_name << "!" << std::endl;
...
void FragTrap::beRepaired(unsigned int amount) {
	...
	if (rprd > this->_hitPoints)
	{
		...
		std::cout << RED << "FR4G-TP" << END << " Earlier " << this->_name << " has hit points (" << this->_hitPoints; 
